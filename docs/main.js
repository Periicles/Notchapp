(function () {
  ...
  var DICT = {
    ...
    "install.3":   { en: "Open NotchBar. macOS blocks it the first time — expected for an app Apple has not notarized.",
                     fr: "Ouvre NotchBar. macOS le bloque la première fois — normal pour une app non notarisée par Apple." },
    ...
    "install.note":{ en: "NotchBar is ad-hoc signed, so Gatekeeper rejects the quarantine flag your browser attached to the download. You do this once; afterwards it opens with a normal double-click. The curl command below skips the step entirely.",
                     fr: "NotchBar n'est pas signée par Apple, donc Gatekeeper refuse l'étiquette de quarantaine posée par ton navigateur. Tu le fais une fois ; ensuite elle s'ouvre d'un double-clic normal. La commande curl ci-dessous évite complètement cette étape." },
    "install.brew":{ en: "Prefer the terminal? Homebrew also keeps NotchBar up to date. The last line clears the quarantine flag Homebrew sets — re-run it after each upgrade. <code>brew tap periicles/tap &amp;&amp; brew trust periicles/tap &amp;&amp; brew install --cask notchbar &amp;&amp;\nxattr -dr com.apple.quarantine /Applications/NotchBar.app</code>",
                     fr: "Plutôt le terminal ? Homebrew garde aussi NotchBar à jour. La dernière ligne retire l'étiquette de quarantaine posée par Homebrew — relance-la après chaque mise à jour. <code>brew tap periicles/tap &amp;&amp; brew trust periicles/tap &amp;&amp; brew install --cask notchbar &amp;&amp;\nxattr -dr com.apple.quarantine /Applications/NotchBar.app</code>" },
